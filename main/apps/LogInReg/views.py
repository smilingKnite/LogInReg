...
def validate():
    if request.session = 'post':
        ...
        session['salt'] = salt
        hashedPw = md5.new(password + salt).hexdigest()
        ...
        if len(email) < 1 or len(password) < 1:
            ...
            if len(result) != 0:
                ...
                encrypted_password = md5.new(
                    password + result[0]['salt']).hexdigest()
...
def add():
    ...
    query = "INSERT INTO users (name, email, psswrd, salt, created_at, updated_at) VALUES (:name, :email, :psswrd, :salt, NOW(), NOW())"
    ...
    flash("Welcome  {}, Thank you for registering!".format(
        request.form['nName']))
